...
function tableToString(t, level)
	...
	if dataType == 'table' then
		...
		for k, v in pairs(t) do
			ret = ret .. echoTabs(level + 1) .. '[' .. tableToString(k) .. ']'
			ret = ret .. ' = '
			ret = ret .. tableToString(v, level + 1)
			ret = ret .. ',\n'
		end
		ret = ret .. echoTabs(level) .. '}'
...
function Logger.dump(value, desciption, nesting)
	local level = Logger.kDebug
	if level < mLevel then
		return
	end
	dump(value, desciption, 10)
end
---
-- 日志的实际输出函数
local function log(level, ...)
	...
	local start, endi = string.find(source,"/?app/")
	if endi ~= nil then
